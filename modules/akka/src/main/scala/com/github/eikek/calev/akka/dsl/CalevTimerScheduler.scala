...
trait CalevTimerScheduler[T] {
  ...
  /** Start an Akka Timer that will send a trigger message produced by `triggerFactory`
    * to the self actor when the upcoming event occurs as defined by given calendar event.
    *
